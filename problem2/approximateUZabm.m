function [u, z] = approximateUZabm(n,rho,x,s)
 global Mf Mg Q c w T dim f g mu sigma;
 cloc=(T-s)*c/T+s;
 wloc=(T-s)*w/T;
 MC=Mg(rho,n+1);
 W=sqrt(T-s)*randn(dim,MC);
 X=repmat(x,1,MC)+mu*(T-s)+sigma*W;
 xi=g(X);
 u=sum(xi,2)/MC;
 ...
 for l=0:(n-1)
  ...
  for k=1:q
   ...
   W=W+dW;
   X=X+mu*d(k)+sigma*dW;
   [U, Z]=cellfun(@approximateUZabm, num2cell(l*ones(1,MC)), num2cell(rho*ones(1,MC)),...
   ...
   if l>0
     [U, Z]=cellfun(@approximateUZabm, num2cell((l-1)*ones(1,MC)), num2cell(rho*ones(1,MC)),...
         num2cell(X,1), num2cell(cloc(k,q)*ones(1,MC)),'UniformOutput',false);
     y=f(cloc(k,q),X,cell2mat(U),cell2mat(Z));
     u=u-wloc(k,q)*sum(y)/MC;
     z=z-wloc(k,q).*sum(repmat(y,dim,1).*W,2)./(MC*(cloc(k,q)-s));
